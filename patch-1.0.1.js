...
processVideoCode = processVideoImportCode;
...
if (!changelogContent.includes('## [1.0.0] - 2024-05-21')) {
...
packageJson.scripts['latest-patch'] = "node .generated/patch-1.0.1.js";
...
execSync(`git commit -m "Apply patch-1.0.0.js: ${process.argv.slice(2).join(' ')}"`, { stdio: 'inherit' });
execSync('git tag 1.0.0', { stdio: 'inherit' });
execSync('git push origin master --tags', { stdio: 'inherit' });

// Создание директории для патчей и следующего патча
...
console.log('Release patch applied. Use "npm test" to run the tests.');
