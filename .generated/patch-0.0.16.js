...
if (!changelogContent.includes('## [0.0.16] - 2024-05-20')) {
...
packageJson.scripts['latest-patch'] = "node patch-0.0.17.js";

if (!packageJson.bin) packageJson.bin = {};
packageJson.bin['otp-cloud'] = "./src/cli.js";

packageJson.version = "0.0.16";

fs.writeFileSync(packageJsonPath, JSON.stringify(packageJson, null, 2));

// Автоматизация git-команд
execSync('git add .', { stdio: 'inherit' });
execSync(`git commit -m "Apply patch-0.0.16.js: ${process.argv.slice(2).join(' ')}"`, { stdio: 'inherit' });

// Создание следующего патча и открытие его в редакторе
const nextPatchNumber = 17;
